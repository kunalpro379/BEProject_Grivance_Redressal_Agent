...
// AI-powered citizen complaints analysis using DeepSeek (with caching)
router.get('/:depId/citizen-complaints-analysis', authenticate, verifyDepartmentAccess, async (req, res) => {
  try {
    const { depId } = req.params;
    ...
    if (!forceRefresh) {
      ...
      if (cachedAnalysis.rows.length > 0) {
        ...
        const analysisData = {
          ...
          totalComplaints: cached.total_complaints,
          averageRating: parseFloat(cached.average_rating) || 0,
          sentimentAnalysis: cached.sentiment_analysis,
          categoryBreakdown: cached.category_breakdown,
          statusDistribution: cached.status_distribution,
          priorityDistribution: cached.priority_distribution,
          topIssues: cached.top_issues,
          geographicHotspots: cached.geographic_hotspots,
          keyInsights: cached.key_insights,
          recommendations: cached.recommendations,
          trends: cached.trends,
          urgentMatters: cached.urgent_matters,
          citizenSatisfaction: cached.citizen_satisfaction,
        ...
        return res.json({
          ...
          data: analysisData,
          cached: true,
          cacheAge: Math.round((Date.now() - new Date(cached.analysis_date).getTime()) / 1000 / 60) + ' minutes'
        });
      }
    }
    ...
    if (grievances.length === 0) {
      return res.json({
        success: true,
        data: {
          summary: 'No complaints data available for analysis.',
          totalComplaints: 0,
          keyInsights: [],
          recommendations: [],
          trends: []
    ...
    try {
      ...
      try {
        ...
        analysisData = {
          ...
          recommendations: ['Review complaints manually for detailed insights']
        };
      }
      ...
      try {
        ...
        const insertResult = await pool.query(
          `INSERT INTO department_complaints_analysis (
            department_id,
            total_complaints,
            average_rating,
            summary,
          ...
          [
            depId,
            analysisData.totalComplaints || complaintsData.length,
            ...
            JSON.stringify(analysisData.statusDistribution || []),
            JSON.stringify(analysisData.priorityDistribution || []),
            JSON.stringify(analysisData.topIssues || []),
            JSON.stringify(analysisData.geographicHotspots || []),
            JSON.stringify(analysisData.keyInsights || []),
            JSON.stringify(analysisData.recommendations || []),
            JSON.stringify(analysisData.trends || []),
            JSON.stringify(analysisData.urgentMatters || []),
            (analysisData.citizenSatisfaction || 'N/A').substring(0, 100) // Truncate to 100 chars
          ]
        );
        ...
        console.error('   Error detail:', saveError.detail);
      ...
      res.json({
        ...
        warning: 'AI analysis unavailable, showing basic statistics',
        cached: false
      });
    }

  } catch (error) {
    console.error('Error analyzing citizen complaints:', error);
    res.status(500).json({ error: 'Failed to analyze citizen complaints' });
  }
});
...
// Reload and save department policies using DeepSeek ReAct Agent
router.post('/:depId/reload-policies', authenticate, verifyDepartmentAccess, async (req, res) => {
  try {
    const { depId } = req.params;
    ...
    res.status(500).json({ 
      success: false,
      message: 'Failed to reload department policies',
      error: error.message 
    });
  }
});
