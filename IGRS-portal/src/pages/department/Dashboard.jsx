...
import {
  ...
  Map as MapIcon, Maximize2, Minimize2, Calendar, Building2, Sparkles, ExternalLink, Image as ImageIcon, Check, XCircle, HardHat, RefreshCw as RefreshIcon
} from 'lucide-react';
...
const DepartmentDashboardNew = () => {
  ...
  const [complaintsAnalysis, setComplaintsAnalysis] = useState(null);
  const [complaintsAnalysisLoading, setComplaintsAnalysisLoading] = useState(false);
  ...
  const loadResourcesData = async () => {
    ...
    try {
      if (activeResourceTab === 'internal-team') {
        ...
        // Also fetch contractor reports
        try {
          const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:4000';
          const reportsRes = await axios.get(`${API_URL}/api/contractor-reports/all`, {
          ...
          console.error('❌ Error fetching contractor reports:', err);
        }
        
        if (res.success) {
          const contractorsData = res.data || [];
          console.log('👷 Fetched Contractors:', contractorsData.length, 'contractors');
  ...
  const loadCitizenFeedback = async () => {
    try {
      ...
      if (analysisRes.success) setComplaintsAnalysis(analysisRes.data);
    } catch (e) {
      const handled = await handleAuthError(e);
      if (!handled) {
        console.error('Error loading citizen feedback:', e);
        setCitizenFeedbackData({ data: [], summary: {} });
        setComplaintsAnalysis(null);
      }
    } finally { 
      setCitizenFeedbackLoading(false);
            ...
            {contractors.map((c, i) => {
              ...
              return (
                <div key={i} className="bg-white rounded-xl border-2 border-stone-200 shadow-md hover:shadow-xl transition-all overflow-hidden">
                  ...
                  {/* Linked Report Text - Primary Display */}
                  {c.linked_report_text && (
                    <div className="border-t-4 border-emerald-500">
                      <div className="p-4 bg-gradient-to-br from-emerald-50 to-teal-50">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <FileText className="w-4 h-4 text-emerald-600" />
                            <h4 className="text-sm font-bold text-emerald-900 uppercase tracking-wide">📋 Featured Report</h4>
                          </div>
                          {c.linked_progress && (
                            <div className="flex items-center gap-2">
                              <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div 
                                  ...
                                  style={{ width: `${c.linked_progress}%` }}
                                ></div>
                              </div>
                              <span className="text-xs font-bold text-emerald-700">{c.linked_progress}%</span>
                            </div>
                          )}
                        </div>

                        {c.linked_project_name && (
                          <div className="bg-white rounded-lg p-2 mb-3 border border-emerald-200">
                            <p className="text-xs font-semibold text-emerald-800">Project: {c.linked_project_name}</p>
                            {c.linked_report_date && (
                              <p className="text-xs text-stone-600 mt-1">
                                Submitted: {new Date(c.linked_report_date).toLocaleDateString()}
                              </p>
                            )}
                          </div>
                        )}

                        <div className="bg-white rounded-lg p-3 border-2 border-emerald-300 max-h-64 overflow-y-auto">
                          <p className="text-xs font-semibold text-emerald-800 mb-2">📄 Complete Report Text:</p>
                          <p className="text-xs text-stone-700 whitespace-pre-wrap leading-relaxed">{c.linked_report_text}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {(() => {
                    const contractorReport = contractorReports.find(r => r.contractor_id === c.id);
                    
                    if (!contractorReport || !contractorReport.recent_reports || contractorReport.recent_reports.length === 0) {
                      return null;
                    }
                    
                    const latestReport = contractorReport.recent_reports[0];
                    const allReports = contractorReport.recent_reports || [];
                    
                    return (
                      <div className="border-t-4 border-[#D4AF37]">
                        {/* Summary Header */}
                        <div className="p-4 bg-gradient-to-r from-amber-100 to-orange-100">
                          ...
                          <div className="grid grid-cols-3 gap-2 mt-3">
                            <div className="bg-white rounded-lg p-2 text-center border-2 border-amber-300">
                              <p className="text-xs text-stone-600">Avg Progress</p>
                              <p className="text-lg font-bold text-amber-700">{Math.round(contractorReport.avg_progress || 0)}%</p>
                            </div>
                            <div className="bg-white rounded-lg p-2 text-center border-2 border-green-300">
                              <p className="text-xs text-stone-600">Total Reports</p>
                              <p className="text-lg font-bold text-green-700">{contractorReport.total_reports}</p>
                            </div>
                            <div className="bg-white rounded-lg p-2 text-center border-2 border-blue-300">
                              <p className="text-xs text-stone-600">Last Update</p>
                              <p className="text-xs font-bold text-blue-700">{new Date(contractorReport.last_report_date).toLocaleDateString()}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="p-4 bg-gradient-to-br from-amber-50 to-orange-50">
                          <div className="flex items-center justify-between mb-3">
                            <p className="text-xs font-bold text-amber-900 uppercase">🎯 Latest Project Report</p>
                            <div className="flex items-center gap-2">
                              <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div 
                                  className="h-full bg-gradient-to-r from-green-500 to-green-600"
                                  style={{ width: `${latestReport.progress_percentage}%` }}
                                ></div>
                              </div>
                              <span className="text-xs font-bold text-green-700">{latestReport.progress_percentage}%</span>
                            </div>
                          </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="bg-white rounded-lg p-3 border border-amber-200">
                              <p className="text-xs font-semibold text-amber-800 mb-1">PROJECT NAME</p>
                              <p className="text-sm font-bold text-stone-900">{latestReport.project_name}</p>
                            </div>

                            <div className="grid grid-cols-2 gap-2">
                              <div className="bg-white rounded-lg p-2 border border-stone-200">
                                <p className="text-xs font-semibold text-stone-600">Contract ID</p>
                                <p className="text-xs font-mono text-stone-900">{latestReport.contract_id}</p>
                              </div>
                              <div className="bg-white rounded-lg p-2 border border-stone-200">
                                <p className="text-xs font-semibold text-stone-600">Submitted</p>
                                <p className="text-xs text-stone-900">{new Date(latestReport.created_at).toLocaleDateString()}</p>
                              </div>
                            </div>
                            ...
                            {latestReport.challenges && (
                              <div className="bg-red-50 rounded-lg p-3 border border-red-200">
                                <p className="text-xs font-semibold text-red-800 mb-1">⚠️ CHALLENGES</p>
                                <p className="text-xs text-red-700">{latestReport.challenges}</p>
                              </div>
                            )}
                            ...
                            {latestReport.next_steps && (
                              <div className="bg-green-50 rounded-lg p-3 border border-green-200">
                                <p className="text-xs font-semibold text-green-800 mb-1">🎯 NEXT STEPS</p>
                                <p className="text-xs text-green-700">{latestReport.next_steps}</p>
                              </div>
                            )}

                            {/* Extracted PDF Content */}
                            {latestReport.extracted_texts && latestReport.extracted_texts.length > 0 && (
                              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-3 border-2 border-blue-300">
                                <p className="text-xs font-bold text-blue-900 mb-2 flex items-center gap-1">
                                  ...
                                  <span className="px-2 py-0.5 bg-blue-200 text-blue-800 rounded-full text-xs">
                                    {latestReport.extracted_texts.length} Document{latestReport.extracted_texts.length !== 1 ? 's' : ''}
                                  </span>
                                </p>
                                {latestReport.extracted_texts.map((text, idx) => (
                                  text && (
                                    <div key={idx} className="mt-2 bg-white rounded p-2 border border-blue-200">
                                      <p className="text-xs font-semibold text-blue-700 mb-1">Document {idx + 1}:</p>
                                      <p className="text-xs text-stone-700 whitespace-pre-wrap">{text.substring(0, 500)}{text.length > 500 ? '...' : ''}</p>
                                    </div>
                                  )
                                ))}
                              </div>
                            ...
                            {latestReport.report_text && (
                              <div className="bg-gradient-to-br from-slate-50 to-gray-50 rounded-lg p-4 border-2 border-slate-300">
                                ...
                                <div className="bg-white rounded-lg p-3 border border-slate-200 max-h-64 overflow-y-auto">
                                  <p className="text-xs text-stone-700 whitespace-pre-wrap leading-relaxed">{latestReport.report_text}</p>
                                </div>
                              </div>
                            )}

                            {/* Document Links */}
                            {latestReport.document_urls && latestReport.document_urls.length > 0 && (
                              <div className="bg-purple-50 rounded-lg p-3 border border-purple-200">
                                <p className="text-xs font-semibold text-purple-800 mb-2">📎 ATTACHED DOCUMENTS</p>
                                <div className="flex flex-wrap gap-2">
                                  {latestReport.document_urls.map((url, idx) => (
                                    <a
                                      key={idx}
                                      href={url}
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="inline-flex items-center gap-1 px-2 py-1 bg-white border border-purple-300 rounded text-xs hover:bg-purple-100 transition-colors"
                                    >
                            ...
                            {latestReport.ai_analysis && (
                              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg p-4 border-2 border-indigo-300">
                                <div className="flex items-center gap-2 mb-3">
                                  ...
                                  <p className="text-sm font-bold text-indigo-900 uppercase">🤖 AI Analysis from Telegram Conversation</p>
                                </div>
                                
                                <div className="grid grid-cols-2 gap-3 mb-3">
                                  {latestReport.ai_analysis.quality_score && (
                                    <div className="bg-white rounded-lg p-3 border border-indigo-200">
                                      <p className="text-xs text-indigo-600 font-semibold mb-1">Quality Score</p>
                                      <div className="flex items-center gap-2">
                                        <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                          <div 
                                            className="h-full bg-gradient-to-r from-indigo-500 to-purple-600"
                                            style={{ width: `${latestReport.ai_analysis.quality_score}%` }}
                                          ></div>
                                        </div>
                                        <span className="text-sm font-bold text-indigo-700">{latestReport.ai_analysis.quality_score}/100</span>
                                      </div>
                                    </div>
                                  )}
                                  
                                  {latestReport.ai_analysis.completeness && (
                                    <div className="bg-white rounded-lg p-3 border border-purple-200">
                                      <p className="text-xs text-purple-600 font-semibold mb-1">Completeness</p>
                                      <div className="flex items-center gap-2">
                                        <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                          <div 
                                            className="h-full bg-gradient-to-r from-purple-500 to-pink-600"
                                            style={{ width: `${latestReport.ai_analysis.completeness}%` }}
                                          ></div>
                                        </div>
                                        <span className="text-sm font-bold text-purple-700">{latestReport.ai_analysis.completeness}/100</span>
                                      </div>
                                    </div>
                                  )}
                                </div>

                                {latestReport.ai_analysis.sentiment && (
                                  <div className="bg-white rounded-lg p-2 border border-indigo-200 mb-3">
                                    <p className="text-xs text-indigo-600 font-semibold">Sentiment: 
                                      <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-bold ${
                                        latestReport.ai_analysis.sentiment === 'positive' ? 'bg-green-100 text-green-800' :
                                        latestReport.ai_analysis.sentiment === 'negative' ? 'bg-red-100 text-red-800' :
                                        'bg-gray-100 text-gray-800'
                                      }`}>
                                        {latestReport.ai_analysis.sentiment.toUpperCase()}
                                      </span>
                                    </p>
                                  </div>
                                )}
                                ...
                                {latestReport.ai_analysis.risk_factors && latestReport.ai_analysis.risk_factors.length > 0 && (
                                  <div className="bg-red-50 rounded-lg p-3 border border-red-200 mb-3">
                                    <p className="text-xs font-bold text-red-800 mb-2">⚠️ Risk Factors Identified:</p>
                                ...
                                {latestReport.ai_analysis.recommendations && latestReport.ai_analysis.recommendations.length > 0 && (
                                  <div className="bg-green-50 rounded-lg p-3 border border-green-200 mb-3">
                                    <p className="text-xs font-bold text-green-800 mb-2">💡 AI Recommendations:</p>
                                    <ul className="space-y-1">
                                      {latestReport.ai_analysis.recommendations.map((rec, idx) => (
                                ...
                                {latestReport.ai_analysis.raw_analysis && !latestReport.ai_analysis.quality_score && (
                                  <div className="bg-white rounded-lg p-3 border border-indigo-200">
                                    <p className="text-xs text-indigo-700 whitespace-pre-wrap">{latestReport.ai_analysis.raw_analysis}</p>
                                  </div>
                                )}
                              </div>
                            )}
                        ...
                        {allReports.length > 1 && (
                          <div className="p-4 bg-stone-50 border-t border-stone-200">
                            <p className="text-xs font-bold text-stone-700 mb-3 uppercase">📋 All Reports History</p>
                            <div className="space-y-2">
                              {allReports.slice(0, 5).map((report, idx) => (
                                <div key={idx} className="bg-white rounded-lg p-2 border border-stone-200 hover:border-amber-300 transition-colors">
                                  <div className="flex items-center justify-between">
                                    <div className="flex-1">
                                      <p className="text-xs font-semibold text-stone-900">{report.project_name}</p>
                                      <p className="text-xs text-stone-600">{report.contract_id}</p>
                                    </div>
                                    <div className="text-right ml-2">
                                      <span className="inline-block px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs font-bold">
                                        {report.progress_percentage}%
                                      </span>
                                      <p className="text-xs text-stone-500 mt-1">{new Date(report.created_at).toLocaleDateString()}</p>
                                    </div>
                                  </div>
                                </div>
                              ))}
                            ...
                            {allReports.length > 5 && (
                              <p className="text-xs text-stone-500 text-center mt-2">
                                + {allReports.length - 5} more reports
                              </p>
                            )}
                          </div>
                        )}
  ...
  const renderCitizenFeedback = () => {
    if (citizenFeedbackLoading || complaintsAnalysisLoading) {
      return (
        <div className="flex items-center justify-center py-24 bg-white rounded-xl border border-stone-200">
          <Loader className="w-10 h-10 animate-spin text-stone-500" />
          <span className="ml-3 text-stone-600">Analyzing citizen complaints with AI...</span>
        </div>
      );
    }

    if (!complaintsAnalysis) {
      return (
        <div className="bg-white rounded-xl border border-stone-200 p-8 text-center">
          <p className="text-stone-500">No complaints data available for analysis</p>
        </div>
      );
    }

    const analysis = complaintsAnalysis;
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Urgent Matters */}
        {analysis.urgentMatters && analysis.urgentMatters.length > 0 && (
          <div className="bg-red-50 rounded-xl border-2 border-red-300 p-6">
            ...
            <div className="space-y-2">
              {analysis.urgentMatters.map((matter, idx) => (
                <div key={idx} className="flex items-start gap-2 p-3 bg-white rounded-lg">
                  <XCircle className="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" />
                  <p className="text-sm text-stone-800">{typeof matter === 'string' ? matter : matter.description || matter.issue}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {analysis.geographicHotspots && analysis.geographicHotspots.length > 0 && (
          <div className="bg-white rounded-xl border-2 border-stone-200 p-6">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {analysis.geographicHotspots.map((hotspot, idx) => (
                <div key={idx} className="p-4 bg-red-50 rounded-lg border border-red-200 text-center">
                  <p className="text-sm font-semibold text-stone-900">{typeof hotspot === 'string' ? hotspot : hotspot.location || hotspot.zone}</p>
                  {typeof hotspot === 'object' && hotspot.count && (
                    <p className="text-xs text-stone-600 mt-1">{hotspot.count} complaints</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {analysis.citizenSatisfaction && (
          <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-xl border-2 border-yellow-300 p-6">
            <h3 className="text-lg font-bold text-yellow-900 mb-3 flex items-center gap-2">
              <Users className="w-5 h-5" />
              Overall Citizen Satisfaction Assessment
            </h3>
            <p className="text-stone-700 leading-relaxed">{analysis.citizenSatisfaction}</p>
          </div>
        )}
      </div>
    );
  ...
  // Reload policies for department
  const handleReloadPolicies = async () => {
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/department-dashboard/${depId}/reload-policies`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (!handled) {
        ...
        alert('Failed to reload policies: ' + (error.response?.data?.message || error.message));
      }
    } finally {
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-stone-800 text-white rounded-lg font-semibold hover:bg-stone-700 transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <div className="bg-white rounded-xl border border-stone-200 shadow-md p-12 text-center">
            ...
            <p className="text-stone-500 text-sm mt-2">Click "Reload Policies" to extract government policies using AI</p>
          </div>
        </div>
      );
    }
