    ...
    <div className="space-y-4">
      ...
      {selectedGrievance && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto border-4 border-black">
            <div className="sticky top-0 bg-black text-white p-4 flex items-center justify-between">
              <h2 className="text-xl font-black uppercase">Grievance Details</h2>
              <button onClick={() => setSelectedGrievance(null)} className="p-2 hover:bg-gray-800 rounded">
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-gray-600 uppercase">Grievance ID</label>
                  <p className="text-lg font-black text-black">{selectedGrievance.id}</p>
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-600 uppercase">Status</label>
                  <div className="mt-1">
                    <span className={`text-sm font-bold px-3 py-1 rounded border ${getStatusColor(selectedGrievance.status)}`}>
              ...
              <div className="border-2 border-gray-200 rounded-lg p-4">
                <h3 className="text-sm font-black text-gray-900 uppercase mb-3">Citizen Information</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center gap-2">
                    <User className="w-4 h-4 text-gray-600" />
                    <div>
                      <p className="text-xs text-gray-500">Name</p>
                      <p className="text-sm font-bold">{selectedGrievance.citizenName}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold">{selectedGrievance.citizenPhone}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold">{selectedGrievance.citizenEmail}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold">{selectedGrievance.location}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-gray-700 leading-relaxed">{selectedGrievance.description}</p>
              </div>

              {/* Workflow Timeline */}
              {selectedGrievance.workflowStages && (
                <div>
                  <h3 className="text-sm font-black text-gray-900 uppercase mb-4">Grievance Workflow Timeline</h3>
                  <div className="space-y-4">
                    {selectedGrievance.workflowStages.map((stage, idx) => (
                      <div key={idx} className={`relative flex gap-4 ${idx !== selectedGrievance.workflowStages.length - 1 ? 'pb-4' : ''}`}>
                        {/* Connector Line */}
                        ...
                        {/* Stage Number/Icon */}
                        <div className={`relative z-10 w-12 h-12 rounded-full flex items-center justify-center font-black text-white border-2 ${
                          stage.status === 'completed' ? 'bg-green-600 border-green-700' :
                          stage.status === 'current' ? 'bg-black border-black' :
                        ...
                        <div className="flex-1">
                          <div className="bg-white border-2 border-gray-200 rounded-lg p-4">
                            <div className="flex items-start justify-between mb-2">
                              <div>
                                <h4 className="text-base font-black text-gray-900 uppercase">{stage.stage}</h4>
                                {stage.timestamp && (
                                  <p className="text-xs text-gray-500 mt-1">
                              ...
                              <span className={`text-xs font-bold px-2 py-1 rounded border ${
                                stage.status === 'completed' ? 'bg-green-50 text-green-900 border-green-200' :
                                stage.status === 'current' ? 'bg-amber-50 text-amber-900 border-amber-200' :
                            ...
                            {stage.officer && (
                              <div className="mb-2">
                                ...
                                <p className="text-sm font-bold text-gray-900">{stage.officer}</p>
                              </div>
                            )}
                            ...
                            {stage.action && stage.action !== 'Pending' && (
                              <div className="mb-2">
                                ...
                                <p className="text-sm font-semibold text-gray-800">{stage.action}</p>
                              </div>
                            )}
                            ...
                            {stage.progressPercentage !== undefined && stage.status === 'current' && (
                              <div className="mb-2">
                                <div className="flex items-center justify-between mb-1">
                                  ...
                                  <p className="text-xs font-bold text-black">{stage.progressPercentage}%</p>
                                </div>
                              ...
                              <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {stage.attachments.map((file, fileIdx) => (
                                    <span key={fileIdx} className="text-xs bg-gray-100 px-2 py-1 rounded border border-gray-300 font-mono">
                                      {file}
                                    </span>
                                  ))}
                                </div>
        ...
        <div key={equipment.equipmentId} className={`bg-white rounded-lg border-2 p-6 ${
          ...
          {equipment.maintenanceOverdue && (
            <div className="bg-red-50 rounded-lg p-4 border-2 border-red-500 mb-4">
              <p className="text-sm font-bold text-red-900">⚠️ Maintenance Overdue by {equipment.overdueBy}</p>
            </div>
          )}
