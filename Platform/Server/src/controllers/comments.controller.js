...
export const addComment = async (req, res) => {
  try {
    ...
    const grievanceCheck = await pool.query(
      'SELECT id, citizen_id FROM usergrievance WHERE id = $1',
      [grievanceId]
    );
...
export const deleteComment = async (req, res) => {
  try {
    ...
    if (!deleted && !isAdmin) {
      return res.status(403).json({ error: 'Failed to delete comment' });
    }

    if (isAdmin && !deleted) {
      await pool.query('DELETE FROM grievancecomments WHERE id = $1', [commentId]);
    }
...
export const getCommentCount = async (req, res) => {
  try {
    ...
    const grievanceCheck = await pool.query(
      'SELECT id FROM usergrievance WHERE id = $1',
      [grievanceId]
    );
