...
OPTIMIZED: Uses Puter AI API for faster validation (no API key needed).
"""
...
class ImageQueryValidator:
    ...
    def validate_image_query_match(
        ...
        """
        Validate if image content matches the grievance query using Puter AI (FAST).
...
"""
            ...
            # On timeout, default to valid to not block legitimate complaints
            return {
                "is_valid": True,
                "validation_score": 0.6,
                "reasoning": "Validation timed out. Defaulting to valid.",
                "mismatches": ["Timeout during validation"],
                "confidence": "low",
                "image_shows": "Timeout error",
            }
