    ...
    try {
      if (isLogin) {
        ...
        if (data.user.role === 'admin') {
          ...
          // ALWAYS use dep_id if available (from database)
          if (data.user.dep_id) {
            redirectUrl = `/department/${data.user.dep_id}`;
            console.log('Redirecting to:', redirectUrl, 'with dep_id:', data.user.dep_id);
          } else {
            // Fallback only if dep_id is not available
        ...
        if (data.requiresApproval) {
          ...
          // If backend requires OTP verification, prompt for OTP once after registration
          if (data.requiresOtpVerification) {
            try {
              const enteredOtp = window.prompt('Enter the OTP sent to your official email to verify your account:');
              if (enteredOtp && formData.email) {
                const otpRes = await fetch(`${API_URL}/api/auth/verify-email-otp`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
              ...
              console.error('OTP verification after registration failed:', otpErr);
            }
          }
      ...
      <div className="relative z-10 w-full max-w-md px-4 sm:px-6 py-8 md:py-12">
        <div className="space-y-8 md:space-y-10">
          ...
          <div className="space-y-6">
            ...
            {/* Registration Link for Department Officers */}
            {isLogin && (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-blue-700 mb-3">
                      Register your department account to access the IGRS portal
                    </p>
                    <button
